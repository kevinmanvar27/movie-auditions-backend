...
@section('content')
    <div class="p-4 sm:p-6">
        ...
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-theme-text">Edit Role</h1>
            <p class="text-theme-text-secondary">Edit role "{{ $role->name }}"</p>
        </div>
            ...
            <form action="{{ route('admin.roles.update', $role) }}" method="POST">
                ...
                <div class="p-4 sm:p-6">
                    <div class="grid grid-cols-1 gap-6">
                        <div>
                            ...
                            <input type="text" name="name" id="name" class="input-field mt-1 block w-full" value="{{ old('name', $role->name) }}" required>
                            @error('name')
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="3" class="input-field mt-1 block w-full">{{ old('description', $role->description) }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-theme-error">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div class="border border-theme-border rounded-lg p-4">
                                    ...
                                    <div class="space-y-2">
                                        <label class="flex items-center">
                                            <input type="checkbox" name="permissions[]" value="manage_users" class="rounded border-theme-border text-theme-primary focus:ring-theme-primary" {{ in_array('manage_users', $role->permissions ?? []) ? 'checked' : '' }}>
                                            <span class="ml-2 text-sm text-theme-text">Manage Users</span>
                                        </label>
                                        <label class="flex items-center">
                                            <input type="checkbox" name="permissions[]" value="manage_roles" class="rounded border-theme-border text-theme-primary focus:ring-theme-primary" {{ in_array('manage_roles', $role->permissions ?? []) ? 'checked' : '' }}>
                                ...
                                <div class="border border-theme-border rounded-lg p-4">
                                    ...
                                    <div class="space-y-2">
                                        <label class="flex items-center">
                                            <input type="checkbox" name="permissions[]" value="manage_movies" class="rounded border-theme-border text-theme-primary focus:ring-theme-primary" {{ in_array('manage_movies', $role->permissions ?? []) ? 'checked' : '' }}>
                                            <span class="ml-2 text-sm text-theme-text">Manage Movies</span>
                                        </label>
                                        <label class="flex items-center">
                                            <input type="checkbox" name="permissions[]" value="manage_auditions" class="rounded border-theme-border text-theme-primary focus:ring-theme-primary" {{ in_array('manage_auditions', $role->permissions ?? []) ? 'checked' : '' }}>
                                ...
                                <div class="border border-theme-border rounded-lg p-4">
                                    ...
                                    <div class="space-y-2">
                                        <label class="flex items-center">
                                            <input type="checkbox" name="permissions[]" value="view_reports" class="rounded border-theme-border text-theme-primary focus:ring-theme-primary" {{ in_array('view_reports', $role->permissions ?? []) ? 'checked' : '' }}>
                                ...
                                <div class="border border-theme-border rounded-lg p-4">
                                    ...
                                    <div class="space-y-2">
                                        <label class="flex items-center">
                                            <input type="checkbox" name="permissions[]" value="view_movies" class="rounded border-theme-border text-theme-primary focus:ring-theme-primary" {{ in_array('view_movies', $role->permissions ?? []) ? 'checked' : '' }}>
                ...
                <div class="px-4 py-3 bg-theme-background border-t border-theme-border flex justify-end space-x-3">
                    ...
                    <button type="submit" class="btn btn-primary px-4 py-2" data-loading>
                        <span class="loading-text">Update Role</span>
